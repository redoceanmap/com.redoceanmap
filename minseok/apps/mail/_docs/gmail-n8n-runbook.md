# Gmail → n8n → 백엔드 실시간 전송 런북 (mail 앱)

> Gmail로 들어온 메일을 실시간(Push) 또는 주기적(Polling)으로 감지해 n8n을 거쳐 자체 백엔드(FastAPI)로 전달하는 시스템 구축 절차. (ragwatson sherlock 런북을 redoceanmap에 맞게 이식)

## ⭐ redoceanmap 적용 현황 (2026-07-02 — Push 방식 가동 중)

| 항목 | 실제 값 |
|---|---|
| 공개 주소 | **Named Tunnel** `https://n8n.redoceanmap.com` (→ host.docker.internal:5678) |
| Pub/Sub | 프로젝트 `study-project-500908`, 토픽 `gmail-push-topic`, 구독 `gmail-push-sub` → `https://n8n.redoceanmap.com/webhook/gmail-push` |
| n8n 워크플로 | "redoceanmap" (id 6ce5lCGGlbuUfqE4) — Webhook(gmail-push) → Get many messages → HTTP Request |
| 백엔드 수신 | `POST http://backend:8000/automation/mail` (허브 자동화 창구, `X-Webhook-Token` 검증) |
| watch 갱신 | Schedule Trigger 매일 12시 → users/me/watch (7일 만료 리셋) |
| 저장 | mail 스포크 `inbound_mails` — **bge-m3 임베딩(vector 1024) 포함 insert**, `/mail/search` 의미 검색 |
...
redoceanmap 실제 구현 (완료):
```
POST /automation/mail  (허브 automation_router — n8n은 허브만 안다)
  헤더: X-Webhook-Token (.env N8N_INBOUND_TOKEN)
  바디: {messageId, subject, sender, recipient, preview}
  → MailIngestInteractor → MailStoragePort → mail 스포크
  → 임베딩(bge-m3) 생성 → inbound_mails insert (중복 message_id 무시)
조회: GET /mail/list · 의미 검색: GET /mail/search?q=
```
- n8n HTTP Request 대상: 같은 compose 네트워크라 `http://backend:8000/...` (서비스 DNS)
