# 🕵️‍♂️ mail 앱: Agent DTO 배역 명세 (설계안)

### (경찰 vs 범인 vs 사설탐정 구조)

본 문서는 `mail` 앱의 `adapter/inbound/api/schemas` 패키지에 도입할 수 있는 12명의 캐릭터 기반 에이전트 DTO 명세서입니다 (ragwatson sherlock_homes에서 이식 — 아직 미구현 설계안). 시스템 내의 상호작용과 대립 구조를 직관적으로 파악할 수 있도록 **[경찰 및 공공기관]**, **[범인 및 전략적 대립자]**, [사설탐정 및 베이커가 조력자]의 3대 구도로 재분류했습니다.
